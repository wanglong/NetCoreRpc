...
namespace NetCoreRpc.Transport.Socketing.Framing
{
    /// <summary>
    /// Copyright (C) 2017 yjq 版权所有。
    /// 类名：LengthPrefixMessageFramer.cs
    /// 类属性：公共类（非静态）
    /// 类功能描述：
    /// 创建标识：yjq 2017/11/24 16:31:05
    /// </summary>
    public class LengthPrefixMessageFramer : IMessageFramer
    {
        ...
        public void Package(IEnumerable<ArraySegment<byte>> data)
        {
            if (data == null)
            {
        ...
        public void Package(ArraySegment<byte> data)
        {
            Parse(data);
        }
        ...
        private void Parse(ArraySegment<byte> bytes)
        {
            ...
            for (int i = bytes.Offset, n = bytes.Offset + bytes.Count; i < n; i++)
            {
                ...
                else
                {
                    ...
                    {
                        LogUtil.Error(string.Format("Parse message buffer failed, _headerLength: {0}, _packageLength: {1}, _bufferIndex: {2}, copyCnt: {3}, _messageBuffer is null: {4}",
                            _headerBytes,
                            _packageLength,
                            _bufferIndex,
                            copyCnt,
                    ...
                    if (_bufferIndex == _packageLength)
                    {
                        if (_receivedHandler != null)
                        {
                            ...
                            {
                                LogUtil.Error("Handle received message fail.", ex);
                            }
                        }
                        _messageBuffer = null;
                        _headerBytes = 0;
